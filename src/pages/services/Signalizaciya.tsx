...
const includedIcons = [Flame, Bell, AlertTriangle, Zap, Building2, CheckCircle2]
const relatedLinks = [
...
export const Signalizaciya = () => {
  ...
  const path = isEn ? "/en/uslugi/signalizaciya" : "/uslugi/signalizaciya"
  const homePath = isEn ? "/en" : "/"

  usePageSeo({
    title: t("service_pages.signalizaciya.seo_title"),
  ...
  const steps = t("service_pages.signalizaciya.steps", { returnObjects: true }) as Step[]
  const objectTypes = t("service_pages.signalizaciya.object_types", { returnObjects: true }) as ObjectType[]
  const relatedLabels = t("service_pages.signalizaciya.related", { returnObjects: true }) as string[]
  ...
  return (
    <ServiceLayout>
      ...
      <nav className="max-w-[1200px] mx-auto px-4 sm:px-6 pt-6 flex items-center gap-1.5 text-xs text-muted-foreground/60" aria-label={t("service_pages.common.breadcrumb_nav")}>
        ...
        <ChevronRight size={11} />
        <span className="text-foreground">{t("service_pages.signalizaciya.breadcrumb_current")}</span>
      </nav>
      ...
      <section className="relative max-w-[1200px] mx-auto px-4 sm:px-6 pt-12 pb-16">
        <motion.div initial={false} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease }} className="max-w-3xl">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <span className="text-[10px] font-mono font-bold uppercase tracking-[0.22em] text-red-600 dark:text-red-500">{t("service_pages.signalizaciya.eyebrow")}</span>
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-[3.5rem] font-black tracking-tight leading-[1.08] mb-6 text-foreground">
            {t("service_pages.signalizaciya.h1_line1")}<br />
            <span className="text-red-600 dark:text-red-500">{t("service_pages.signalizaciya.h1_accent")}</span><br />
            {t("service_pages.signalizaciya.h1_line3")}
          </h1>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed max-w-xl mb-8">
            {t("service_pages.signalizaciya.intro")}
          </p>
      ...
      <section className="border-t border-border/30 py-16">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <h2 className="text-2xl sm:text-3xl font-black tracking-tight mb-10 text-foreground">{t("service_pages.signalizaciya.object_types_title")}</h2>
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="relative rounded-3xl bg-red-600 p-10 sm:p-14 overflow-hidden">
            ...
            <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div>
                <h2 className="text-2xl sm:text-3xl font-black text-white mb-2">{t("service_pages.signalizaciya.cta_title")}</h2>
                <p className="text-red-100/80 text-sm">{t("service_pages.signalizaciya.cta_desc")}</p>
              </div>
