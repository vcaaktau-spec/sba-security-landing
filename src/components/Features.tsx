...
export const Features = () => {
  ...
  const wrapperVariants = {
    ...
    visible: {
      opacity: 1, 
      y: 0, 
      transition: { duration: 1, ease: smoothEase as any, delay: 0.1 }
    }
  }
  ...
  const cardVariants = {
    ...
    visible: (i: number) => ({
      opacity: 1, 
      scale: 1,
      transition: { duration: 0.8, ease: smoothEase as any, delay: 0.3 + i * 0.1 }
    })
  }
  ...
  return (
    <section
      id="features"
      ref={containerRef}
      className="relative min-h-screen snap-start flex flex-col justify-center py-20 lg:py-24 overflow-hidden bg-background"
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1240px] px-4 sm:px-6 flex flex-col">
        
        {/* === ЗАГОЛОВОК === */}
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
          <motion.h2 
            variants={textVariants} initial="hidden" animate={isInView ? "visible" : "hidden"}
          ...
          <motion.p
            variants={textVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} transition={{ delay: 0.1 }}
            className="text-base sm:text-lg text-muted-foreground"
          >
            Узнайте, почему нам доверяют безопасность самых сложных объектов в Актау.
          </motion.p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-5 lg:gap-6 h-full">
            {featuresData.map((feature, i) => (
              ...
              >
                ...
                {/* Декоративная линия снизу (вместо кнопки) */}
